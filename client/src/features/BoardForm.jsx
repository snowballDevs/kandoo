...
const BoardForm = ({handleClose}) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await fetch(
                'http://localhost:6969/boards/createBoard/',
                {
                    // to be changed later on with axios
                    method: 'POST',
                    headers: {
    ...
    return (
        <div className='fixed -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 min-w-[75%] '>
            <form
                className='modal-box mx-auto form-control w-full'
                onSubmit={handleSubmit}
            >
                <label
                    htmlFor='boardName'
                    className='label label-text block mb-2 font-bold text-lg'
                >
                ...
                <button
                    type='submit'
                    className='btn btn-outline border-2 border-indigo-800'
                >
                    Submit
                </button>
            </form>
